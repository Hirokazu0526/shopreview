<template>
  <div class="shop-entry">
    <h2>ショップ登録</h2>
    <div v-if="shop.image">
      <img :src="shop.image" alt="" class="preview-image" />
    </div>
    <div class="entry-input-row">
      <span class="entry-label">画像</span
      ><input v-if="reset" @change="upload" type="file" />
    </div>
    <div class="entry-input-row">
      <span class="entry-label">店名</span
      ><input type="text" v-model="shop.shopName" />
    </div>
    <div class="entry-input-row">
      <span class="entry-label">店舗紹介</span
      ><textarea v-model="shop.description" row="5"></textarea>
    </div>
    <div>
    ...
    <div class="message">
      <div v-if="entryDocId">
        FirebaseにDocId： {{ entryDocId }} で登録しました。
      </div>
